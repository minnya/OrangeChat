...
class MessageModelHelper {
  ...
  //　送信処理
  Future<bool> submit({required RoomModel roomModel, String? message, String? imageUrl}) async{
    // roomModelのroomIdが0の場合は、roomがないので作成する
    if(roomModel.roomId==0){
      roomModel.setRoomId(await RoomModelHelper().createRoom(userId: roomModel.userId));
    }
    // メッセージを送信する
    final result = await client.from("chat_messages")
        .insert([{
      "sender": uid,
  ...
  // 既読処理
  Future<List<MessageModel>> updateRead(RoomModel roomModel, List<MessageModel> messageList) async{
    print("roomID: ${roomModel.roomId}");
    print("uid: $uid");
    final result = await client.from("chat_messages")
        .update({ 'is_read': true })
        .match({"room": roomModel.roomId, "receiver": uid, "is_read": false})
        .select("*");
    return messageList;
  }
}
