...
class PostModelHelper {
  ...
  Future<List<PostModel>> getAll(
    ...
    if (keyword.isNotEmpty) {
      postQuery = postQuery.textSearch(
          "message", keyword.replaceAll(" ", " & "),
          type: TextSearchType.plain, config: "simple");
    }

    List<Map<String, dynamic>> posts = await postQuery.not(
        "id", "in", await UserModelHelper().getBlocks()); // Blockしたユーザーの投稿を除外
    ...
    List dislikeList = (await client
        ...
        .toList();
    for (PostModel post in postList) {
      post.disliked = dislikeList.contains(post.id);
    }
    return postList;
  }

  Future<bool> putPost(
      {required String message, List<String>? imageUrls}) async {
    try {
  ...
  Future<bool> putLike(int postId) async {
    return await putReview(postId, 1);
  }

  // Likeを削除する
  Future<void> deleteLike(int postId) async {
    return await deleteReview(postId, 1);
  }

  // Likeを追加する
  Future<bool> putDislike(int postId) async {
    return await putReview(postId, -1);
  }
  ...
  // Reviewを追加する
  Future<bool> putReview(int postId, int score) async {
    final result = await client.from('post_review').upsert(
      {'user': uid, 'post': postId, "score": score},
    ).select("*");
    return result.isNotEmpty;
  }

  // Reviewを削除する
  Future<void> deleteReview(int postId, int score) async {
    await client
        .from('post_review')
        .delete()
        .match({'user': uid, 'post': postId, "score": score});
  }
}
